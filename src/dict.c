 ...
 *    of elements and the buckets <= 1 / (HASHTABLE_MIN_FILL *
...
#define ENTRY_PTR_IS_ODD_KEY                                                   \
  1 /* XX1 : If a pointer to odd key address (must be 1). */
#define ENTRY_PTR_IS_EVEN_KEY                                                  \
  2 /* 010 : If a pointer to even key address. (must be 2 or 4). */
#define ENTRY_PTR_NO_VALUE                                                     \
  4 /* 100 : If a pointer to an entry without value.                           \
     */
...
void dictTypeAddMeta(dict **d, dictType *typeWithMeta) {
  ...
  toCmp.onDictRelease =
      (*d)->type->onDictRelease; /* Ignore 'onDictRelease' in comparison */
  assert(memcmp((*d)->type, &toCmp, sizeof(dictType)) ==
         0); /* The rest of the dictType fields must be the same */
 ...
 * case it'll be set to 1). Returns DICT_OK if resize was performed, and
 * DICT_ERR if skipped. 可以缩容也可以扩容
 */
int _dictResize(dict *d, unsigned long size, int *malloc_failed) {
   ...
   * 每次都会设置第二个哈希表方便进行 rehash, 如果是第一次初始化,
   * 之后代码会清空这个 hash 表 初始化第二个哈希表的已使用桶数量（初始为0）
   * 将新分配的哈希表指针赋值给第二个表
   * */
...
int _dictExpand(dict *d, unsigned long size, int *malloc_failed) {
  ...
  if (dictIsRehashing(d) || d->ht_used[0] > size ||
      DICTHT_SIZE(d->ht_size_exp[0]) >= size)
    return DICT_ERR;
...
/* Helper function for `dictRehash` and `dictBucketRehash` which rehashes all
 * the keys in a bucket at index `idx` from the old to the new hash HT. idx 0 号
 * hash 表的桶的索引号
 */
...
/* This checks if we already rehashed the whole table and if more rehashing is
 * required */
...
/*
 * 每一步重哈希指的是将旧哈希表中的一个桶（bucket）
 * 迁移到新表中。由于哈希表采用链地址法（chaining） 处理冲突，
 * 一个桶可能包含多个键（通过链表连接），因此一步操作可能迁移多个键。
 * 函数最多会扫描 N*10
 * 个空桶。这是为了避免在哈希表存在大量空桶时，函数因无意义的扫描而长时间阻塞（保证操作的时间复杂度可控）。
 */
...
int dictRehash(dict *d, int n) {
   ...
   * - If shrinking, the threshold is 1 / (HASHTABLE_MIN_FILL *
   * dict_force_resize_ratio) which is 1/32. */
 ...
 * Note:
 * Ensure that the key's uniqueness is managed externally before calling this
 * function. */
 ...
 *
 * dictAddRaw 主要2个函数
 * dictFindPositionForInsert 获取要插入的 position
 * dictInsertAtPosition  添加一个 dictEntry 到 hash 表中
 */
dictEntry *dictInsertAtPosition(dict *d, void *key, void *position) {
  ...
  assert(bucket >= &d->ht_table[htidx][0] &&
         bucket <=
             &d->ht_table[htidx][DICTHT_SIZE_MASK(d->ht_size_exp[htidx])]);
  if (d->type->no_value) {
    if (!*bucket) {
...
dictEntry *dictFindByHash(dict *d, const void *key, const uint64_t hash) {
  ...
  // 使用哈希值与第一个哈希表的掩码进行按位与操作，得到在第一个哈希表中的桶索引。
  idx = hash & DICTHT_SIZE_MASK(d->ht_size_exp[0]);
  keyCmpFunc cmpFunc = dictGetKeyCmpFunc(d);
  ...
  /* Check if we can use the compare function with length to avoid recomputing
   * length of key always */
  ...
  for (table = 0; table <= 1; table++) {
    ...
    while (he) {
      ...
      if (key == he_key || (has_len_fn ? cmpFuncWithLen(d, key, key_len, he_key,
                                                        keyLenFunc(d, he_key))
 ...
 * `dictTwoPhaseUnlinkFree` with it in order to unlink and release it. Otherwise
 * if the key is not found, NULL is returned. These two functions should be used
 * in pair. `dictTwoPhaseUnlinkFind` pauses rehash and `dictTwoPhaseUnlinkFree`
 * resumes rehash.
 *
 ...
 * dictTwoPhaseUnlinkFree(db->dict,de,plink,table); // We don't need to lookup
 * again
 *
...
void dictTwoPhaseUnlinkFree(dict *d, dictEntry *he, dictEntry **plink,
                            int table_index) {
  if (he == NULL)
    return;
...
size_t dictEntryMemUsage(void) { return sizeof(dictEntry); }
...
dictEntry *dictGetRandomKey(dict *d) {
   ...
   * 获得了一个非空的 bucket, 先计算这个 bucket 下链表的长度,
   * 再从中随机选取一个节点
   */
  listlen = 0;
  orighe = he;
  ...
  while (stored < count && maxsteps--) {
    for (j = 0; j < tables; j++) {
      ...
      if (he == NULL) {
        ...
        emptylen = 0;
        while (he) {
          /* Collect all the elements of the buckets found non empty while
           * iterating. To avoid the issue of being unable to sample the end of
           * a long chain, we utilize the Reservoir Sampling algorithm to
           * optimize the sampling process. This means that even when the
           * maximum number of samples has been reached, we continue sampling
           * until we reach the end of the chain. See
...
static void dictDefragBucket(dictEntry **bucketref,
 ...
 *     在 ht[0] 遍历到某个元素后,该元素迁移到新表 ht[1] 中, SCAN
 * 会再次遇到该元素,导致重复返回 2) The iterator must return multiple elements
 * per call, as it needs to always return all the keys chained in a given
 * bucket, and all the expansions, so we are sure we don't miss keys moving
 * during rehashing. 3) The reverse cursor is somewhat hard to understand at
 * first, but this comment is supposed to help.
 */
/*
 * 下一个游标的操作为 反转 +1 再反转
 * 000  0
 ...
 * 000 会 hash 到 0000/1000 ,即用索引 0(000) 去遍历表0和表1 的 0(000)
 * 号桶,可遍历到所有原来0号桶的元素 100 会 hash 到 0100/1100 ,即用索引 4(100)
 * 去遍历表0和表1 的 4(100) 号桶,可遍历到所有原来0号桶的元素
 */
 ...
 * valid. */
unsigned long dictScanDefrag(dict *d, unsigned long v, dictScanFunction *fn,
  ...
  if (!dictIsRehashing(d)) {
    ...
    if (DICTHT_SIZE(d->ht_size_exp[htidx0]) >
        DICTHT_SIZE(d->ht_size_exp[htidx1])) {
      htidx0 = 1;
      htidx1 = 0;
    }
...
/* Returning DICT_OK indicates a successful expand or the dictionary is
 * undergoing rehashing, and there is nothing else we need to do about this
 * dictionary currently. While DICT_ERR indicates that expand has not been
 * triggered (may be try shrinking?)*/
int dictExpandIfNeeded(dict *d) {
  ...
  if ((dict_can_resize == DICT_RESIZE_ENABLE &&
      ...
      (dict_can_resize != DICT_RESIZE_FORBID &&
       d->ht_used[0] >=
           dict_force_resize_ratio * DICTHT_SIZE(d->ht_size_exp[0]))) {
...
/* Returning DICT_OK indicates a successful shrinking or the dictionary is
 * undergoing rehashing, and there is nothing else we need to do about this
 * dictionary currently. While DICT_ERR indicates that shrinking has not been
 * triggered (may be try expanding?)*/
int dictShrinkIfNeeded(dict *d) {
  /* Incremental rehashing already in progress. Return.
   * 如果字典正在进行重哈希，则直接返回DICT_OK，不执行收缩操作
   */
  if (dictIsRehashing(d))
    return DICT_OK;

  /* If the size of hash table is DICT_HT_INITIAL_SIZE, don't shrink it.
   * 如果当前哈希表大小小于或等于初始大小(DICT_HT_INITIAL_SIZE，默认为4)，则不进行收缩，返回DICT_OK
   */
  if (DICTHT_SIZE(d->ht_size_exp[0]) <= DICT_HT_INITIAL_SIZE)
    return DICT_OK;

  /* If we reached below 1:8 elements/buckets ratio, and we are allowed to
   * resize the hash table (global setting) or we should avoid it but the ratio
   * is below 1:32, we'll trigger a resize of the hash table.
   * DICT_RESIZE_ENABLE 模式
   *    已使用元素数 × 8 ≤ 桶数量
   * DICT_RESIZE_FORBID 模式
   *    已使用元素数 × 8 × 4 ≤ 桶数量
   */
  if ((dict_can_resize == DICT_RESIZE_ENABLE &&
       d->ht_used[0] * HASHTABLE_MIN_FILL <= DICTHT_SIZE(d->ht_size_exp[0])) ||
      (dict_can_resize != DICT_RESIZE_FORBID &&
       d->ht_used[0] * HASHTABLE_MIN_FILL * dict_force_resize_ratio <=
...
/*
 * 两种情况都是迁移整个 bucket
 * (1) visitedIdx 大于已经rehash过的 rehashidx, 且对应 bucket 有效,则迁移整个
 * bucket (2) visitedIdx 小于rehashidx, 或者 bucket 为空,则执行 dictRehash(d,1),
 * 其实就是查找下一个有效的 bucket,迁移整个 bucket
 */
static void _dictRehashStepIfNeeded(dict *d, uint64_t visitedIdx) {
...
void *dictFindPositionForInsert(dict *d, const void *key,
                                dictEntry **existing) {
  unsigned long idx, table;
  dictEntry *he;
...
dictEntry *dictFindByHashAndPtr(dict *d, const void *oldptr,
                                const uint64_t hash) {
  dictEntry *he;
  unsigned long idx, table;
...
/* Provides the old and new ht size for a given dictionary during rehashing.
 * This method should only be invoked during initialization/rehashing. */
void dictRehashingInfo(dict *d, unsigned long long *from_size,
...
void dictCombineStats(dictStats *from, dictStats *into) {
  ...
  into->maxChainLen = (from->maxChainLen > into->maxChainLen)
                          ? from->maxChainLen
                          : into->maxChainLen;
...
size_t dictGetStatsMsg(char *buf, size_t bufsize, dictStats *stats, int full) {
  ...
  l += snprintf(buf + l, bufsize - l,
                ...
                stats->htidx,
                (stats->htidx == 0) ? "main hash table" : "rehashing target",
  ...
  if (full) {
    l += snprintf(buf + l, bufsize - l,
                  ...
                  (float)stats->totalChainLen / stats->buckets,
...
void print_dict(dict *d) {
  ...
  printf("Dictionary buckets: %lu\n", dictBuckets(d));
  printf("Dictionary rehashidx: %ld\n", d->rehashidx);

  printf("\nRehash status: %s\n",
         dictIsRehashing(d) ? "In progress" : "Not in progress");
  if (dictIsRehashing(d)) {
    printf("Rehash index: %ld\n", d->rehashidx);
    printf("Table 0 size: %lu, used: %lu\n", DICTHT_SIZE(d->ht_size_exp[0]),
           d->ht_used[0]);
    printf("Table 1 size: %lu, used: %lu\n", DICTHT_SIZE(d->ht_size_exp[1]),
           d->ht_used[1]);
  }

  // 打印每个哈希表中的bucket数据
  for (int table = 0; table < 2; table++) {
    if (DICTHT_SIZE(d->ht_size_exp[table]) == 0)
      continue;

    printf("\n---------- Hash Table %d --------------\n", table);
    printf("Size: %lu, Used: %lu\n", DICTHT_SIZE(d->ht_size_exp[table]),
           d->ht_used[table]);

    // 遍历哈希表的每个bucket
    for (unsigned long i = 0; i < DICTHT_SIZE(d->ht_size_exp[table]); i++) {
      dictEntry *he = d->ht_table[table][i];

      // 如果bucket不为空，打印bucket信息
      if (he != NULL) {
        printf("Bucket %lu: ", i);

        // 遍历bucket中的链表
        while (he != NULL) {
          const char *key = (const char *)dictGetKey(he);
          long value = (long)dictGetVal(he);
...
#define MAX_STRING_SIZE 500
  ...
  size_t substringSize =
      MIN_STRING_SIZE + (rand() % (MAX_STRING_SIZE - MIN_STRING_SIZE + 1));
...
dictType BenchmarkDictType = {hashCallback, NULL, NULL, compareCallback,
                              freeCallback, NULL, NULL};
...
int dictTest(int argc, char **argv, int flags) {
  ...
  TEST("Use DICT_RESIZE_AVOID to disable the dict resize and pad to "
       "(dict_force_resize_ratio * 16)") {
  ...
  TEST("Add one more key, trigger the dict resize") {
    retval = dictAdd(d, stringFromLongLong(current_dict_used),
                     (void *)(current_dict_used));
    assert(retval == DICT_OK);
    current_dict_used++;
  ...
  TEST("Delete keys until we can trigger shrink in next test") {
    /* Delete keys until we can satisfy (1 / HASHTABLE_MIN_FILL) in the next
     * test. */
    for (j = new_dict_size / HASHTABLE_MIN_FILL + 1;
         j < (long)current_dict_used; j++) {
  ...
  TEST("Use DICT_RESIZE_AVOID to disable the dict resize and reduce to 3") {
    ...
    dictSetResizeEnabled(DICT_RESIZE_AVOID);
    remain_keys = DICTHT_SIZE(d->ht_size_exp[0]) /
  ...
  TEST("Test dict Rehash ") {
    ...
    // 创建一个不释放键值的字典类型
    dictType testDictType = {hashCallback,
                             NULL, // keyDup - 不复制键
                             NULL, // valDup - 不复制值
                             compareCallback,
                             NULL, // keyDestructor - 不释放键
                             NULL, // valDestructor - 不释放值
                             NULL,
                             NULL};

    dict *d0 = dictCreate(&testDictType);

    // 插入所有测试键
    for (int i = 0; i < 4; i++) {
      // 使用键的索引作为值
      dictAdd(d0, (void *)test_keys[i], (void *)(long)i);
    }

    printf("Inserted %d keys into dictionary\n", num_keys);
    print_dict(d0);

    printf("add key4\n");
    dictAdd(d0, (void *)test_keys[4], (void *)(long)4);

    print_dict(d0);

    printf("add key5\n");
    dictAdd(d0, (void *)test_keys[5], (void *)(long)5);

    print_dict(d0);

    printf("add key6\n");
    dictAdd(d0, (void *)test_keys[6], (void *)(long)6);

    print_dict(d0);

    printf("add key7\n");
    dictAdd(d0, (void *)test_keys[7], (void *)(long)7);

    print_dict(d0);

    printf("add key8\n");
    dictAdd(d0, (void *)test_keys[8], (void *)(long)8);

    print_dict(d0);

    printf("add key9\n");
    dictAdd(d0, (void *)test_keys[9], (void *)(long)9);

    print_dict(d0);

    printf("add key10\n");
    dictAdd(d0, (void *)test_keys[10], (void *)(long)10);

    print_dict(d0);

    printf("add key11\n");
    dictAdd(d0, (void *)test_keys[11], (void *)(long)11);

    print_dict(d0);

    printf("add key12\n");
    dictAdd(d0, (void *)test_keys[12], (void *)(long)12);

    print_dict(d0);

    printf("add key13\n");
    dictAdd(d0, (void *)test_keys[13], (void *)(long)13);

    print_dict(d0);

    printf("del key13 key12 key11 key10 ... key4\n");
    dictDelete(d0, (void *)test_keys[13]);
    dictDelete(d0, (void *)test_keys[12]);
    dictDelete(d0, (void *)test_keys[11]);
    dictDelete(d0, (void *)test_keys[10]);
    dictDelete(d0, (void *)test_keys[9]);
    dictDelete(d0, (void *)test_keys[8]);
    dictDelete(d0, (void *)test_keys[7]);
    dictDelete(d0, (void *)test_keys[6]);
    dictDelete(d0, (void *)test_keys[5]);
    dictDelete(d0, (void *)test_keys[4]);
    print_dict(d0);

    printf("del key3\n");
    dictDelete(d0, (void *)test_keys[3]);
    print_dict(d0);
    printf("del key2\n");
    dictDelete(d0, (void *)test_keys[2]);
    print_dict(d0);
    printf("del key1\n");
    dictDelete(d0, (void *)test_keys[1]);
    print_dict(d0);
    printf("del key0\n");
    dictDelete(d0, (void *)test_keys[0]);
    print_dict(d0);

    dictRelease(d0);
  }

  TEST("Test dict Rehash end")
  ...
  for (j = 0; j < count; j++) {
    ...
    /* If key already exists NULL is returned so we need to free the temp key
     * string */
    if (de == NULL)
      zfree(key);
  }
  ...
  end_benchmark(
      "Find() and inserting via "
      "dictGetHash()+dictFindByHash()+dictAddNonExistsByHash() non existing");
  assert((long)dictSize(d) == count);
