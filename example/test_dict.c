...
// 测试用的键复制函数
void *testKeyDup(dict *d, const void *key) { return strdup((char *)key); }

// 测试用的值复制函数
void *testValDup(dict *d, const void *obj) { return strdup((char *)obj); }

// 测试用的键析构函数
void testKeyDestructor(dict *d, void *key) { free(key); }

// 测试用的值析构函数
void testValDestructor(dict *d, void *obj) { free(obj); }

// 字典类型定义
dictType testDictType = {.hashFunction = testHashFunction,
                         .keyDup = testKeyDup,
                         .valDup = testValDup,
...
void assert_str_equal(const char *expected, const char *actual,
                      const char *msg) {
...
void test_dict_find_same_bucket_keys() {
  printf("Testing dict find keys with same bucket...\n");

  // 使用不同的键来测试哈希冲突
  const char *test_keys[] = {"key0",  "key1",  "key2",  "key3",  "key4",
  ...
  // 假设哈希表大小为4 (2^2)
  const int ht_size_exp = 2;
  const int bucket_count = 1 << ht_size_exp; // 4个bucket
  BucketKeys *buckets = calloc(bucket_count, sizeof(BucketKeys));

  // 初始化buckets
  for (int i = 0; i < bucket_count; i++) {
    buckets[i].capacity = 10;
    buckets[i].keys = malloc(buckets[i].capacity * sizeof(char *));
    buckets[i].count = 0;
  }

  printf("Finding keys that map to the same buckets (hash table size 2^%d = "
         "%d):\n",
         ht_size_exp, bucket_count);

  // 计算每个键的bucket索引并分组
  for (int i = 0; i < num_keys; i++) {
    uint64_t hash_v = dictGenHashFunction(test_keys[i], strlen(test_keys[i]));
    unsigned long idx = hash_v & DICTHT_SIZE_MASK(ht_size_exp);

    printf("Key: '%s' -> bucket: %lu (hash: %llu)\n", test_keys[i], idx,
           (unsigned long long)hash_v);

    // 将键添加到对应的bucket中
    BucketKeys *bucket = &buckets[idx];
    if (bucket->count >= bucket->capacity) {
      bucket->capacity *= 2;
      bucket->keys = realloc(bucket->keys, bucket->capacity * sizeof(char *));
    }
    bucket->keys[bucket->count] = strdup(test_keys[i]);
    bucket->count++;
  }

  printf("\nKeys grouped by buckets:\n");
  // 显示每个bucket中的键
  for (int i = 0; i < bucket_count; i++) {
    if (buckets[i].count > 0) {
      printf("Bucket %d: ", i);
      for (int j = 0; j < buckets[i].count; j++) {
        printf("'%s' ", buckets[i].keys[j]);
      }
      printf("\n");

      // 如果这个bucket有多个键，说明存在哈希冲突
      if (buckets[i].count > 1) {
        printf("  -> COLLISION: %d keys in same bucket!\n", buckets[i].count);
      }
    }
  }
...
void test_dict_replace() {
  ...
  assert_equal_int(0, result,
                   "Replace should indicate existing key was replaced");
  ...
  assert_str_equal("new_value2", (char *)value,
                   "New key should have correct value");
...
void test_dict_integer_values() {
  ...
  assert_equal_int(42, dictGetSignedIntegerVal(entry),
                   "Should get correct integer value");

  // 增加值
  dictIncrSignedIntegerVal(entry, 8);
  assert_equal_int(50, dictGetSignedIntegerVal(entry),
                   "Should get incremented value");

  // 测试无符号整数
  char *key2 = strdup("unsigned_counter");
  entry = dictAddRaw(d, key2, NULL);
  dictSetUnsignedIntegerVal(entry, 1000ULL);
  assert_equal_int(1000, dictGetUnsignedIntegerVal(entry),
                   "Should get correct unsigned value");

  dictIncrUnsignedIntegerVal(entry, 500ULL);
  assert_equal_int(1500, dictGetUnsignedIntegerVal(entry),
                   "Should get incremented unsigned value");
...
void test_dict_double_values() {
  ...
  assert_true(value > 3.14158 && value < 3.14160,
              "Should get correct double value");

  // 增加值
  dictIncrDoubleVal(entry, 0.1);
  value = dictGetDoubleVal(entry);
  assert_true(value > 3.24158 && value < 3.24160,
              "Should get incremented double value");
...
void test_dict_rehash() {
  ...
  assert_true(steps >= 0,
              "Microsecond rehash should return non-negative steps");
...
int main() {
  ...
  //
  test_dict_find_same_bucket_keys();
